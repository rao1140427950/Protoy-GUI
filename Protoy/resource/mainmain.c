...
void main()
{
    ...
    {   
        ...
        //else SendString("Nothing recieved.\r\n");
        /*if (peripherals[2] == 0x03)
        {
            temp = rd_humi(0x02);
            SendByte(temp);
            Delay1000ms();
...
void CheckPeripherals()
{
    ...
    for (i = 2; i < MAX_CHECKID; i++)  // 检查外设是否online
    {
        if(peripherals[i] < ONLINE_DIV)
        {
            ...
            //SendByte(i);
            //SendArray(dat, n);
            if(n != 1 || dat[0] != CHECK_ONLINE) peripherals[i] |= ONLINE_DIV;  // 如果该外设已经凉了
        }
    }
    RefreshEEPROM();
}
...
void MountPeripheral(uint8 label)  // 为新接入的外设分配ID
{
    ...
    for(i = 2; i < MAX_PERIPHERALS; i++)
    {
        if(peripherals[i] >= ONLINE_DIV)
        {
            ...
            dat[1] = i;
            if(BusSend(dat, 0x00, 2) == 1) RefreshEEPROM();   // 使用保留地址作广播，收到ACK则更新外设库
            else peripherals[i] |= ONLINE_DIV;  // 没有收到ACK则unmount
            break;            
        }
    }
}
